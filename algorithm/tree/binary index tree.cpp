...
// Method 4 BIT with space compression
// sort the nums first, then map the val to the sorted nums
// input: [5,2,6,1]
// sorted [1,2,5,6]
// mapped value to index: 5 -> 3, 2 -> 2, 6 -> 4, 1 -> 1,  
class BIT{
public:
    BIT(int n): d_bit(n+1) {
    }
    ...
    vector<int> countSmaller(vector<int>& nums) {
        ...
        for(int i = n-1; i >= 0; --i){
            ...
            if(nums[i] != nums_sort[0]){
                int smaller_idx = lower_bound(nums_sort.begin(), nums_sort.end(), nums[i]) - nums_sort.begin();
                ans[i] = bit.get(smaller_idx);
            }
            int cur_idx = lower_bound(nums_sort.begin(), nums_sort.end(), nums[i]) - nums_sort.begin() + 1;
            bit.update(cur_idx, 1);
        }
        return ans;
    }
};
